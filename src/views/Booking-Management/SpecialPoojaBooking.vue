<template>
  ...
  <!-- <AddPoojaBooking
    ...
    :formData="formData"
    :kovilSpecialPoojaTypes="kovilSpecialPoojaTypes"
...
<NoData :items="items" />
        <tbody v-for="(item, i) in itemsLocal" :key="i">
          ...
          <tr class="intro-x">
            <td
              :style="
                opened.includes(item.specialPoojaBookingId)
                  ? getSelectedColor()
                  : 'color: black'
              "
            >
            ...
            <td
              ...
              "
            >
              {{ getKovilSpecialPoojaById(item.kovilSpecialPoojasId) }}
            </td>
            <td
              :style="
                opened.includes(item.specialPoojaBookingId)
            ...
            <td class="table-report__action">
              <div class="flex justify-end items-end">
                <ArrowDownCircleIcon
                  v-if="!opened.includes(item.specialPoojaBookingId)"
          ...
          <tr v-if="opened.includes(item.specialPoojaBookingId)">
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "Special Pooja Bookings");
  },
  components: {},
  data() {
    return {
      cName: "SpecialPoojaBooking",
      opened: [],
      isLoading: false,
      searchData: "",
      searchActive: false,
  ...
  methods: {
    ...
    toggle(item) {
      ...
      this.indexToggle = this.opened.indexOf(item.specialPoojaBookingId);
      if (this.indexToggle > -1) {
        this.opened.splice(this.indexToggle, 1);
      } else {
        this.viewCustomerInfo(item.specialPoojaBookingId);
        this.opened.push(item.specialPoojaBookingId);
      }
    },
    closeToggle(item) {
      this.customerInfo = [];
    ...
    viewCustomerInfo(specialPoojaBookingId) {
      try {
        var newArray = [];
        newArray = this.itemsLocal.find(
          (x) => x.specialPoojaBookingId === specialPoojaBookingId
        );
    ...
    getKovilPoojaTypes() {
      axios
        .post("kovilSpecialPoojas/findAll")
        .then((response) => {
          if (response.data.success) {
            this.kovilSpecialPoojaTypes = response.data.data;
          } else {
            warningToast(response.data.message);
          }
        })
    ...
    getKovilSpecialPoojaById(value) {
      try {
        var result = this.kovilSpecialPoojaTypes.find(
          (x) => x.kovilSpecialPoojasId === (value)
        );
        var poojaName = result.specialPooja.name;
        return poojaName;
      } catch {
        return "Null";
      }
    ...
    archiveItemConfirm() {
      var itemId = this.formData.specialPoojaBookingId;
      this.isLoading = true;
      axios
