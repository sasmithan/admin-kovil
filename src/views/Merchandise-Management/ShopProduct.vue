<template>
  ...
  <AddShopProduct @modalClose="closeForm($event)" @getData="getDataFromApi($event)" :modalStatus="addForm"
    :titleChange="titleChange" :formData="formData" />
  ...
  <div class="md:-mt-11 w-full">
    <div
      ...
      <div class="flex space-x-1">
        <AddItem @parentMethod="showModal($event)" buttonName="Add Shop Product" />
          ...
          <tr v-for="(item, i) in itemsLocal" :key="i" class="intro-x">
            ...
            <td>
              <div
                  ...
                  :style="{ 'background-color': getStatusColor(item.isDiscount) }"
                >
                  {{ getDiscountStatus(item.isDiscount) }}
                </div>
            </td>
            <td>
              <div
                  class="text-white rounded-2xl p-1 pl-2 pr-2 flex items-center justify-center whitespace-nowrap"
                  :style="{ 'background-color': getStatusColor(item.isVisible) }"
                >
                  {{ getDiscountStatus(item.isVisible) }}
                </div>
            </td>
            <td>
              <div
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "Shop Product");
  },
  components: {},
  data() {
    return {
  ...
  methods: { // get items
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("ShopProduct/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("ShopProduct/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
        ...
        axios
          .post("ShopProduct/findAllArchive")
          .then((response) => {
            if (response.data.success) {
              this.archiveItems = response.data.data;
              this.archiveItemsCount = this.archiveItems.length;
    ...
    archiveItemConfirm() {
      var itemId = this.formData.shopProductId;
      this.isLoading = true;
      axios
    ...
    getShopById(value) {
      try {
        return this.shopList.find((x) => x.shopId === parseInt(value)).shopName;
      } catch {
        return "Nil";
      }
    },
