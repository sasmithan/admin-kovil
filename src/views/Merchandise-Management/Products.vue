  ...
  <div class="md:-mt-11 w-full">
    ...
    >
      <div class="flex space-x-1">
        <AddItem @parentMethod="showModal($event)" buttonName="Add Product" />
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("Product/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("Product/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
    ...
    archiveItemConfirm() {
      var itemId = this.formData.productId;
      this.isLoading = true;
      axios
        .post("Product/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
