  ...
  <div class="md:-mt-11 w-full">
    ...
    >
      <div class="flex space-x-1">
        <AddItem @parentMethod="showModal($event)" buttonName="Add Natchaththiram" />
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "நட்சத்திரங்கள்");
  },
  components: {},
  data() {
    return {
      ...
      titleChange: false,
      formData: { name: "" },
      formDataDefault: { name: "" },

      headers: [
      { text: "ID", value: "natchaththiramId" },
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("Natchaththiram/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("Natchaththiram/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
        ...
        axios
          .post("Natchaththiram/findAllArchive")
          .then((response) => {
            if (response.data.success) {
              this.archiveItems = response.data.data;
              this.archiveItemsCount = this.archiveItems.length;
    ...
    archiveItemConfirm() {
      var itemId = this.formData.natchaththiramId;
      this.isLoading = true;
      axios
        .post("Natchaththiram/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
    ...
    restoreItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .post("Natchaththiram/restore/" + itemId)
        .then((response) => {
          if (response.data.success) {
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("Natchaththiram/delete/" + itemId)
        .then((response) => {
          if (response.data.success) {
