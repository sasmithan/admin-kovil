  ...
  <div class="md:-mt-11 w-full">
    ...
    >
      <div class="flex space-x-1">
        <AddItem @parentMethod="showModal($event)" buttonName="Add Jothidar" />
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "ஜோதிடர்கள்");
  },
  components: {},
  data() {
    return {
  ...
  methods: {
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("Jothidar/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("Jothidar/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
    ...
    archiveItemConfirm() {
      var itemId = this.formData.jothidarId;
      this.isLoading = true;
      axios
        .post("Jothidar/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
    ...
    async getJothidarCategory() {
      await axios
        .post("JothidarCategory/findAll")
        .then((response) => {
          this.jothidarCategoryList = response.data.data;
        })
        .catch((error) => {
          warningToast(statusMessage(error.response.status));
        });
    },
    getJothidarCategoryById(value) {
      try {
        return this.jothidarCategoryList.find(
          (x) => x.jothidarCategoryId === parseInt(value)
        ).name;
      } catch {
        return "Nil";
      }
