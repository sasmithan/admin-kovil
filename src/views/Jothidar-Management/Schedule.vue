  ...
  <div class="md:-mt-11 w-full">
    ...
    >
      <div class="flex space-x-1">
        <AddItem @parentMethod="showModal($event)" buttonName="Add Schedule Time" />
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", " நேர அட்டவணை");
  },
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("Schedule/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("Schedule/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
    ...
    archiveItemConfirm() {
      var itemId = this.formData.scheduleId;
      this.isLoading = true;
      axios
        .post("Schedule/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
