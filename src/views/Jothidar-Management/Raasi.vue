<template>
    ...
    <div class="md:-mt-11 w-full">
      ...
      >
        <div class="flex space-x-1">
          <AddItem @parentMethod="showModal($event)" buttonName="Add Raasi" />
  ...
  export default {
    mounted() {
      store.dispatch("menuTitle/setSubMenuTitle", "ராசிகள்");
    },
    components: {},
    data() {
      return {
    ...
    methods: {
      ...
      getDataFromApi() {
        ...
        if (this.searchActive) {
          axios
            .get("Raasi/search/" + this.searchData)
            .then((response) => {
      ...
      getArchiveData() {
        ...
        if (this.searchActiveArchive) {
          axios
            .get("Raasi/archiveSearch/" + this.searchDataArchive)
            .then((response) => {
              if (response.data.success) {
                if (response.data.data != null) {
      ...
      archiveItemConfirm() {
        var itemId = this.formData.raasiId;
        this.isLoading = true;
        axios
          .post("Raasi/archive/" + itemId)
          .then((response) => {
            if (response.data.success) {
