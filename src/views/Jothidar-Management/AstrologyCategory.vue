    ...
    >
      <div class="flex space-x-1">
        <AddItem
          ...
          buttonName="Add Astrology Category"
        />
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "ஜோதிடர் வகைகள்");
  },
  components: {},
  data() {
    return {
  ...
  methods: {
   ...
   getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("AstrologyCategory/search/" + this.searchData)
          .then((response) => {
        ...
        axios
          .post("AstrologyCategory/findAll")
          .then((response) => {
            if (response.data.success) {
              this.items = response.data.data;
              this.itemsCount = this.items.length;
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("AstrologyCategory/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
        ...
        axios
          .post("AstrologyCategory/findAllArchive")
          .then((response) => {
            if (response.data.success) {
              this.archiveItems = response.data.data;
              this.archiveItemsCount = this.archiveItems.length;
    ...
    archiveItemConfirm() {
      var itemId = this.formData.astrologyCategoryId;
      this.isLoading = true;
      axios
        .post("AstrologyCategory/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
    ...
    restoreItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .post("AstrologyCategory/restore/" + itemId)
        .then((response) => {
          if (response.data.success) {
    ...
    deleteItemConfirm() {
      ...
      axios
        .delete("AstrologyCategory/delete/" + itemId)
        .then((response) => {
          if (response.data.success) {
