  ...
  <div class="md:-mt-11 w-full">
    ...
    >
      <div class="flex space-x-1">
        <AddItem @parentMethod="showModal($event)" buttonName="Add God" />
        <UploadFile
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "கடவுள்கள்");
  },
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("Gods/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("Gods/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
    ...
    archiveItemConfirm() {
      var itemId = this.formData.godId;
      this.isLoading = true;
      axios
        .post("Gods/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
