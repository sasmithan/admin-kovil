<template>
  ...
  export default {
    mounted() {
      store.dispatch("menuTitle/setSubMenuTitle", "பூஜை நேரங்கள்");
    },
    components: {},
    data() {
      return {
    ...
    methods: {
      ...
      getDataFromApi() {
        ...
        if (this.searchActive) {
          axios
            .get("PoojaTime/search/" + this.searchData)
            .then((response) => {
      ...
      getArchiveData() {
        ...
        if (this.searchActiveArchive) {
          axios
            .get("PoojaTime/archiveSearch/" + this.searchDataArchive)
            .then((response) => {
              if (response.data.success) {
                if (response.data.data != null) {
      ...
      archiveItemConfirm() {
        var itemId = this.formData.poojaTimeId;
        this.isLoading = true;
        axios
          .post("PoojaTime/archive/" + itemId)
          .then((response) => {
            if (response.data.success) {
      ...
      deleteItemConfirm() {
        ...
        axios
          .delete("PoojaTime/delete/" + itemId)
          .then((response) => {
            if (response.data.success) {
