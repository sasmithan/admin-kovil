...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "கோவில் வகைகள்");
  },
  components: {},
  data() {
    return {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      if (this.searchActive) {
        axios
          .get("Category/search/" + this.searchData)
          .then((response) => {
    ...
    getArchiveData() {
      ...
      if (this.searchActiveArchive) {
        axios
          .get("Category/archiveSearch/" + this.searchDataArchive)
          .then((response) => {
            if (response.data.success) {
              if (response.data.data != null) {
    ...
    setPagination() {
      ...
      var pageCountRound = Math.round(pageCount);
    ...
    archiveItemConfirm() {
      var itemId = this.formData.categoryId;
      this.isLoading = true;
      axios
        .post("Category/archive/" + itemId)
        .then((response) => {
          if (response.data.success) {
