<template>
  ...
  <Modal :show="addCategory" @hidden="addCategory = false" backdrop="static">
    <ModalHeader>
    ...
    <ModalBody class="flex justify-center space-y-4">
      <div class="flex flex-col space-y-4 w-full">
        ...
        <div>
          <label for="regular-form-1" class="form-label">Category Code</label>

          <div class="flex flex-col space-y-1">
            <input
              id="vertical-form-1"
              v-model="formDataCategory.code"
              type="text"
              class="form-control"
              name="code"
              placeholder="123"
        ...
        <div>
          ...
          <div class="flex flex-col space-y-1">
            <textarea
              ...
              class="form-control"
              name="description"
              placeholder="Type here about Category..."
            ></textarea>
          </div>
        </div>
        <div class="mt-3">
          ...
          <div class="form-switch mt-2">
            <input
              type="checkbox"
              v-model="formDataCategory.visible"
              class="form-check-input"
            />
          </div>
        </div>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.titleChange === -1 ? "Add New Category" : "Edit Category";
    },
  },
  created() {
    this.getDataFromApi();
  ...
  methods: {
    ...
    pdfDownload() {
      ...
      this.docs.save(`${"Categories " + generateDate}.pdf`);
    },
    ...
    csvDownload() {
      if (this.allItems <= 0) {
        ...
        var generateDate = new Date().toLocaleString();
        var csvTitle = "Categories " + generateDate;
    ...
    setDefault() {
      this.formDataCategory = Object.assign({}, this.formDataCategoryDefault);
    },
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("Categories/Paginated", this.pagination)
        .then((response) => {
          if (response.data.success === true) {
    ...
    editItem(item) {
      this.titleChange = 0;
      this.formDataCategory = Object.assign({}, item);
      this.addCategory = true;
    },
    ...
    saveForm() {
      if (
        ...
        this.formDataCategory.code == ""
      ) {
        if (this.formDataCategory.name == "") {
          warningToast("Name Required");
        }
        if (this.formDataCategory.code == "") {
        ...
        if (this.titleChange === -1) {
          this.isLoading = true;
          axios
            .post("Categories", this.formDataCategory)
            .then((response) => {
              if (response.data.success === true) {
                this.addCategory = false;
                this.getDataFromApi();
          ...
          var itemId = this.formDataCategory.categoryId;
          this.isLoading = true;
          axios
            .put("Categories/" + itemId, this.formDataCategory)
            .then((response) => {
              if (response.data.success === true) {
                this.addCategory = false;
                this.getDataFromApi();
    ...
    viewArchives() {
      this.isLoading = true;
      axios
        .post("Categories/Archived", this.paginationArchive)
        .then((response) => {
          if (response.data.success === true) {
