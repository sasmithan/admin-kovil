<template>
  ...
  <MoveableModal
    :show="addPickerModal"
    @hidden="addPickerModal = false"
    backdrop="static"
    size="modal-lg"
    :title="formTitle"
    @close="addPickerModal = false"
  >
    <ModalBody class="grid grid-cols-2 lg:gap-4 md:gap-4 gap-2">
      ...
      <div class="w-full">
        ...
        <TomSelect v-model="formDataUser.warehouseId" class="w-full">
          <option
            :value="formDataUser.warehouseId"
            v-if="titleChange != -1 && formDataUser.warehouse != ''"
          >
            {{ formDataUser.warehouse }}
          </option>
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "Pickers");
  },
  components: {
    Loading,
    paginate: Paginate,
  ...
  methods: {
    ...
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("Pickers/Paginated", this.pagination)
    ...
    getAllDataFromApi() {
      ...
      this.isLoading = true;
      axios
        .post("Pickers/Paginated", pagination)
        .then((response) => {
          if (response.data.success === true) {
    ...
    saveForm() {
      ...
      if (
        ...
        this.formDataUser.warehouseId == "" ||
        this.formDataUser.warehouseId == "SELECT" ||
        this.formDataUser.blockTypeId == "" ||
        this.formDataUser.blockTypeId == "SELECT" ||
        ...
        if (
          this.formDataUser.warehouseId == "" ||
          this.formDataAddSku.warehouseId == "SELECT"
        ) {
          warningToast("Warehouse Required");
        }
        if (
          this.formDataUser.blockTypeId == "" ||
          this.formDataAddSku.blockTypeId == "SELECT"
        ) {
          warningToast("Block Type Required");
        }
        ...
        if (this.titleChange === -1) {
          this.isLoading = true;
          axios
            .post("Pickers", this.formDataUser)
            .then((response) => {
              if (response.data.success === true) {
                this.addPickerModal = false;
                this.getDataFromApi();
          ...
          this.isLoading = true;
          axios
            .put("Pickers/" + itemId, this.formDataUser)
            .then((response) => {
              if (response.data.success === true) {
                this.addPickerModal = false;
                this.getDataFromApi();
    ...
    viewArchives() {
      this.isLoading = true;
      axios
        .post("Pickers/Archived", this.paginationArchive)
        .then((response) => {
          if (response.data.success === true) {
