  ...
  <div class="mt-5 w-full">
    ...
    >
      <div class="lg:flex space-x-1 sm:grid-col-12">
        ...
        <div>
          <a
            v-if="reportData.customerId != '' && reportData.skuId != ''"
            href="javascript:;"
            @click="showReports()"
...
export default {
  ...
  methods: {
    ...
    showReports() {
      ...
      if (customerId == "SELECT" || skuId == "SELECT") {
        if (customerId == "SELECT") {
          failedToast("Please Select Customer");
        }
        if (skuId == "SELECT") {
          failedToast("Please Select SKU Description");
        }
      } else {
        this.getReports();
      }
    ...
    getReports() {
      ...
      if (item.customerId != "") {
        axios
          .get(
            `ReportFinal/MonthlyProductSales?FromDate=${item.fromdate}&ToDate=${item.todate}&SKUDiscription=${item.skuId}&customerId=${item.customerId}`
          )
          .then((response) => {
            if (response.data.success === true) {
