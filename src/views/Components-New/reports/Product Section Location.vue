      ...
      <div class="lg:flex space-x-1 sm:grid-col-12">
        <div>
          ...
          >
            ...
            <option :value="item.name" v-for="(item, i) in sections" :key="i">
              {{ item.name }}
            </option>
          </TomSelect>
        </div>
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "Product Section Locations Reports");
  },
  components: {
    paginate: Paginate,
  },
  ...
  methods: {
    ...
    pdfDownload() {
      ...
      this.docs.save(`${"Product Select Location Report " + generateDate}.pdf`);
    },
      ...
      axios
        ...
        .then((response) => {
          if (response.data.success === true) {
            this.sections = response.data.data.items;
          } else {
            this.isLoading = false;
