  ...
  >
    ...
    <ModalBody class="flex justify-center space-y-4">
      <div class="flex flex-col space-y-4 w-full">
        <div>
          <label for="regular-form-1" class="form-label">Pallet Type</label>

          <div class="flex flex-col space-y-1">
            <input
              id="vertical-form-1"
              v-model="formDataPalletType.name"
              type="text"
              class="form-control"
              name="name"
...
export default {
  ...
  methods: {
    ...
    pdfDownload() {
      ...
      this.docs.save(`${"Pallet Type " + generateDate}.pdf`);
    },
    ...
    csvDownload() {
      if (this.allItems <= 0) {
        ...
        var generateDate = new Date().toLocaleString();
        var csvTitle = "Pallet Type " + generateDate;
    ...
    getAllDataFromApi() {
      ...
      this.isLoading = true;
      axios
        .post("PalletTypes/Paginated", pagination)
        .then((response) => {
    ...
    setDefault() {
      this.formDataPalletType = Object.assign(
        {},
        this.formDataPalletTypeDefault
      );
    },
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("PalletTypes/Paginated", this.pagination)
        .then((response) => {
          if (response.data.success === true) {
    ...
    editItem(item) {
      this.titleChange = 0;
      this.formDataPalletType = Object.assign({}, item);
      this.addPalletSize = true;
    },

    saveForm() {
      if (this.formDataPalletType.name == "") {
        if (this.formDataPalletType.name == "") {
        ...
        if (this.titleChange === -1) {
          this.isLoading = true;
          axios
            .post("PalletTypes", this.formDataPalletType)
            .then((response) => {
              if (response.data.success === true) {
                this.addPalletSize = false;
                this.getDataFromApi();
          ...
          var itemId = this.formDataPalletType.palletTypeId;
          this.isLoading = true;
          axios
            .put("PalletTypes/" + itemId, this.formDataPalletType)
            .then((response) => {
              if (response.data.success === true) {
                this.addPalletSize = false;
                this.getDataFromApi();
    ...
    archiveItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("PalletTypes/" + itemId + "/Archive")
        .then((response) => {
          if (response.data.success === true) {
            this.archiveConfirmationModal = false;
            this.getDataFromApi();
    ...
    viewArchives() {
      this.isLoading = true;
      axios
        .post("PalletTypes/Archived", this.paginationArchive)
        .then((response) => {
          if (response.data.success === true) {
    ...
    restoreItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("PalletTypes/" + itemId + "/Restore")
        .then((response) => {
          if (response.data.success === true) {
            this.restoreConfirmationModal = false;
            this.viewArchives();
