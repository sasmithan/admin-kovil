<template>
  ...
  <!-- BEGIN: Add  Modal -->
  <!-- <Modal :show="addType" @hidden="addType = false" backdrop="static">
    <ModalHeader>
    ...
    <ModalBody class="flex justify-center space-y-4">
      <div class="flex flex-col space-y-4 w-full">
        <div>
          <label for="regular-form-1" class="form-label">Type Name</label>
        ...
        <div>
          <label for="regular-form-1" class="form-label"
            >Storage Type Abbreviation</label
          >

          <div class="flex flex-col space-y-1">
            <input
              id="vertical-form-1"
              v-model="formDataType.abbreviation"
              type="text"
              class="form-control"
              name="abbreviation"
        ...
        <div>
          ...
          <div class="flex flex-col space-y-1">
            <textarea
              id="vertical-form-1"
              v-model="formDataType.description"
              type="text"
              class="form-control"
              name="description"
  ...
  <AddStorageType
    :titleStatus="titleChange"
    :formData="formDataType"
    :modalStatus="addType"
    :modalConfirm="saveForm"
...
export default {
  ...
  methods: {
    ...
    pdfDownload() {
      ...
      this.docs.save(`${"Storage Types " + generateDate}.pdf`);
    },
    ...
    csvDownload() {
      if (this.allItems <= 0) {
        ...
        var generateDate = new Date().toLocaleString();
        var csvTitle = "Storage Types " + generateDate;
    ...
    setDefault() {
      this.formDataType = Object.assign({}, this.formDataTypeDefault);
    },
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("StorageTypes/Paginated", this.pagination)
        .then((response) => {
          if (response.data.success === true) {
    ...
    editItem(item) {
      this.titleChange = 0;
      this.formDataType = Object.assign({}, item);
      this.addType = true;
    },

    saveForm() {
      if (
        this.formDataType.name == "" ||
        this.formDataType.abbreviation == ""
      ) {
        if (this.formDataType.name == "") {
          warningToast("Name Required");
        }
        if (this.formDataType.abbreviation == "") {
        ...
        if (this.titleChange === -1) {
          this.isLoading = true;
          axios
            .post("StorageTypes", this.formDataType)
            .then((response) => {
              if (response.data.success === true) {
                this.addType = false;
                this.getDataFromApi();
          ...
          var itemId = this.formDataType.storageTypeId;
          this.isLoading = true;
          axios
            .put("StorageTypes/" + itemId, this.formDataType)
            .then((response) => {
              if (response.data.success === true) {
                this.addType = false;
                this.getDataFromApi();
    ...
    archiveItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("StorageTypes/" + itemId + "/Archive")
        .then((response) => {
          if (response.data.success === true) {
            this.archiveConfirmationModal = false;
            this.getDataFromApi();
    ...
    viewArchives() {
      this.isLoading = true;
      axios
        .post("StorageTypes/Archived", this.paginationArchive)
        .then((response) => {
          if (response.data.success === true) {
    ...
    restoreItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("StorageTypes/" + itemId + "/Restore")
        .then((response) => {
          if (response.data.success === true) {
            this.restoreConfirmationModal = false;
            this.viewArchives();
