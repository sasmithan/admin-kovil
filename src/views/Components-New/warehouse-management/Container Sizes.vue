<template>
  ...
  <!-- BEGIN: Add container size Modal -->

  <Modal
    :show="addContainerSize"
    @hidden="addContainerSize = false"
    backdrop="static"
  >
    <ModalHeader>
    ...
    <ModalBody class="flex justify-center space-y-4">
      <div class="flex flex-col space-y-4 w-full">
        <div>
          <label for="regular-form-1" class="form-label">Container Size</label>

          <div class="flex flex-col space-y-1">
            <input
              id="vertical-form-1"
              v-model="formDataConSize.name"
              type="text"
              class="form-control"
              name="name"
...
export default {
  ...
  methods: {
    ...
    pdfDownload() {
      ...
      this.docs.save(`${"Container sizes " + generateDate}.pdf`);
    },
    ...
    csvDownload() {
      if (this.allItems <= 0) {
        ...
        var generateDate = new Date().toLocaleString();
        var csvTitle = "Container Size " + generateDate;
    ...
    getAllDataFromApi() {
      ...
      this.isLoading = true;
      axios
        .post("ContainerSizes/Paginated", pagination)
        .then((response) => {
    ...
    loadMoreData() {
      ...
      this.pageSize = this.itemsData.pageNumber;
      this.getDataFromApi();
    },
    pageSizeChange() {
    ...
    setDefault() {
      this.formDataConSize = Object.assign({}, this.formDataConSizeDefault);
    },
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("ContainerSizes/Paginated", this.pagination)
        .then((response) => {
          if (response.data.success === true) {
    ...
    showModal() {
      this.setDefault();
      this.titleChange = -1;
      this.addContainerSize = true;
    },
    closeForm() {
      this.addContainerSize = false;
    },
    editItem(item) {
      this.titleChange = 0;
      this.formDataConSize = Object.assign({}, item);
      this.addContainerSize = true;
    },

    saveForm() {
      if (this.formDataConSize.name == "") {
        if (this.formDataConSize.name == "") {
        ...
        if (this.titleChange === -1) {
          this.isLoading = true;
          axios
            .post("ContainerSizes", this.formDataConSize)
            .then((response) => {
              if (response.data.success === true) {
                this.addContainerSize = false;
                this.getDataFromApi();
          ...
          var itemId = this.formDataConSize.containerSizeId;
          this.isLoading = true;
          axios
            .put("ContainerSizes/" + itemId, this.formDataConSize)
            .then((response) => {
              if (response.data.success === true) {
                this.addContainerSize = false;
                this.getDataFromApi();
    ...
    archiveItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("ContainerSizes/" + itemId + "/Archive")
        .then((response) => {
          if (response.data.success === true) {
            this.archiveConfirmationModal = false;
            this.getDataFromApi();
    ...
    viewArchives() {
      this.isLoading = true;
      axios
        .post("ContainerSizes/Archived", this.paginationArchive)
        .then((response) => {
          if (response.data.success === true) {
    ...
    restoreItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("ContainerSizes/" + itemId + "/Restore")
        .then((response) => {
          if (response.data.success === true) {
            this.restoreConfirmationModal = false;
            this.viewArchives();
