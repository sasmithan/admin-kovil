<template>
  ...
  <!-- BEGIN: Add container type  Modal -->

  <Modal
    :show="addContainerType"
    @hidden="addContainerType = false"
    backdrop="static"
  >
    <ModalHeader>
    ...
    <ModalBody class="flex justify-center space-y-4">
      <div class="flex flex-col space-y-4 w-full">
        <div>
          <label for="regular-form-1" class="form-label">Container Type</label>

          <div class="flex flex-col space-y-1">
            <input
              id="vertical-form-1"
              v-model="formDataConType.name"
              type="text"
              class="form-control"
              name="name"
...
export default {
  ...
  methods: {
    ...
    pdfDownload() {
      ...
      this.docs.save(`${"Conner Type " + generateDate}.pdf`);
    },
    ...
    csvDownload() {
      if (this.allItems <= 0) {
        ...
        var generateDate = new Date().toLocaleString();
        var csvTitle = "Conner Type " + generateDate;
    ...
    getAllDataFromApi() {
      ...
      this.isLoading = true;
      axios
        .post("ContainerTypes/Paginated", pagination)
        .then((response) => {
    ...
    setDefault() {
      this.formDataConType = Object.assign({}, this.formDataConTypeDefault);
    },
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("ContainerTypes/Paginated", this.pagination)
        .then((response) => {
          if (response.data.success === true) {
    ...
    showModal() {
      this.setDefault();
      this.titleChange = -1;
      this.addContainerType = true;
    },
    closeForm() {
      this.addContainerType = false;
    },
    editItem(item) {
      this.titleChange = 0;
      this.formDataConType = Object.assign({}, item);
      this.addContainerType = true;
    },

    saveForm() {
      if (this.formDataConType.name == "") {
        if (this.formDataConType.name == "") {
        ...
        if (this.titleChange === -1) {
          this.isLoading = true;
          axios
            .post("ContainerTypes", this.formDataConType)
            .then((response) => {
              if (response.data.success === true) {
                this.addContainerType = false;
                this.getDataFromApi();
          ...
          var itemId = this.formDataConType.containerTypeId;
          this.isLoading = true;
          axios
            .put("ContainerTypes/" + itemId, this.formDataConType)
            .then((response) => {
              if (response.data.success === true) {
                this.addContainerType = false;
                this.getDataFromApi();
    ...
    archiveItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("ContainerTypes/" + itemId + "/Archive")
        .then((response) => {
          if (response.data.success === true) {
            this.archiveConfirmationModal = false;
            this.getDataFromApi();
    ...
    viewArchives() {
      this.isLoading = true;
      axios
        .post("ContainerTypes/Archived", this.paginationArchive)
        .then((response) => {
          if (response.data.success === true) {
    ...
    restoreItemConfirm() {
      ...
      this.isLoading = true;
      axios
        .get("ContainerTypes/" + itemId + "/Restore")
        .then((response) => {
          if (response.data.success === true) {
            this.restoreConfirmationModal = false;
            this.viewArchives();
