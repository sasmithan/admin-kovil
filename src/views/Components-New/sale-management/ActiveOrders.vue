        ...
        <table class="table table-report -mt-2 table-auto">
          ...
          <tbody v-for="(item, i) in items" :key="i">
            <tr class="intro-x">
              ...
              <td
                :style="opened.includes(item.orderId) ? getSelectedColor() : ''"
              >
            ...
            <tr v-if="opened.includes(item.orderId)">
              <td colspan="12">
                <table class="table table-report -mt-2 table-auto">
                  ...
                  <tbody>
                    <tr v-show="orderLineArray.length <= 0">
                      <td colspan="6">
                        <span
                          class="
                            flex
                    ...
                    <tr
                      v-for="(item, index) in orderLineArray"
                      :key="index"
                      class="intro-x"
                    >
                      <td>
                      ...
                      <td>
                        {{
                          item.discountedReason != null
                            ? item.discountedReason
                            : "Nil"
                        }}
                      </td>
...
export default {
  mounted() {
    store.dispatch("menuTitle/setSubMenuTitle", "Active Orders");
  },
  components: {
    Moveable,
    paginate: Paginate,
  ...
  methods: {
    toggle(item) {
      ...
      if (this.indexToggle > -1) {
        ...
        this.viewOrderLines(item.orderId);
    ...
    getDataFromApi() {
      this.isLoading = true;
      axios
        .post("OrderPickingQueue/ActiveOrders/DFLT", this.pagination)
        .then((response) => {
    ...
    connectTimetoDate() {
      ...
      var deliveryDate = moment(this.formDataActive.deliveryDate).format(
        "YYYY-MM-DD"
      );
    ...
    deleteItemConfirm() {
      this.isLoading = true;
      var itemId = this.formDataActive.customerOrderId;
