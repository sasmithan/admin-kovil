...
export default {
  ...
  computed: {
    formTitle() {
      return this.titleChange === false ? "Add Pooja Type" : "Edit Pooja Type";
    },
  },
  ...
  methods: {
    async saveForm() {
      this.saving = true;
      await axios
        .post("PoojaType/Create", this.formData)
        .then((response) => {
    ...
    async updateForm() {
      ...
      var id = this.formData.poojaTypeId;
      this.updating = true;
      await axios
        .put("PoojaType/edit/" + id, this.formData)
        .then((response) => {
          if (response.data.success) {
            this.modalClose();
            this.getDataFromApi();
