    ...
    <ModalBody class="">
      <div class="grid">
        <div class="w-full mb-2">
          ...
          <v-select
            v-model="formData.poojaTimeId"
            placeholder="SELECT"
            class="style-chooser cursor-pointer"
            :options="poojaTimes"
            :reduce="(name) => name.poojaTimeId"
            label="time"
          />
        </div>
      </div>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.titleChange === false
        ? "Add Kovil Pooja Time"
        : "Edit Kovil Pooja Time";
    },
  },
  ...
  methods: {
    ...
    async saveForm() {
      ...
      this.saving = true;
      await axios
        .post("KovilPoojaTime/Create", this.formData)
        .then((response) => {
          if (response.data.success) {
            this.modalClose();
            this.getDataFromApi();
    ...
    async updateForm() {
      var id = this.formData.kovilPoojaTimeId;

      this.updating = true;
      await axios

        .put("KovilPoojaTime/edit/" + id, this.formData)
        .then((response) => {
          if (response.data.success) {
            this.modalClose();
            this.getDataFromApi();
