    ...
    <ModalBody class="">
      <div class="grid">
        <div class="w-full mb-2">
          <label for="regular-form-1" class="form-label">Special Pooja</label>
...
export default {
  ...
  methods: {
    async saveForm() {
      this.saving = true;
      await axios
        .post("SpecialPooja/Create", this.formData)
        .then((response) => {
    ...
    async updateForm() {
      ...
      var id = this.formData.specialPoojaId;
      this.updating = true;
      await axios
        .put("SpecialPooja/edit/" + id, this.formData)
        .then((response) => {
          if (response.data.success) {
            this.modalClose();
            this.getDataFromApi();
