    ...
    <ModalBody class="">
      <div class="grid">
        <div class="w-full mb-2">
          <label for="regular-form-1" class="form-label">God Name</label>
...
export default {
  ...
  computed: {
    formTitle() {
      return this.titleChange === false ? "Add God" : "Edit God";
    },
  },
  ...
  methods: {
    async saveForm() {
      this.saving = true;
      await axios
        .post("Gods/Create", this.formData)
        .then((response) => {
    ...
    async updateForm() {
      ...
      var id = this.formData.godId;
      this.updating = true;
      await axios
        .put("Gods/edit/" + id, this.formData)
        .then((response) => {
          if (response.data.success) {
            this.modalClose();
            this.getDataFromApi();
