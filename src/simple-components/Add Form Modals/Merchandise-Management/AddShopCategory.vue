...
export default {
  ...
  computed: {
    formTitle() {
      return this.titleChange === false ? "Add Shop Category" : "Edit Shop Category";
    },
  },
  ...
  methods: {
    async saveForm() {
      ...
      this.saving = true;
      await axios
        .post("ShopCategory/Create", this.formData)
        .then((response) => {
    ...
    async updateForm() {
      ...
      var id = this.formData.shopCategoryId;
      this.updating = true;
      await axios
        .put("ShopCategory/Edit/" + id, this.formData)
        .then((response) => {
          if (response.data.success) {
            this.modalClose();
            this.getDataFromApi();
